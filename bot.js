...
const welcomeKeyboard = {
    reply_markup: {
        inline_keyboard: [
            [
                { text: "📢 Join Channel", url: config.TELEGRAM_CHANNEL },
                { text: "👥 Join Group", url: config.TELEGRAM_GROUP }
            ],
            [
...
const mainMenuKeyboard = {
    reply_markup: {
        inline_keyboard: [
            [
                { text: "💣 START BOMB", callback_data: "start_bomb" },
                { text: "📊 MY STATS", callback_data: "my_stats" }
            ],
            [
                { text: "👨‍💻 ABOUT DEV", callback_data: "about" },
...
bot.on('callback_query', async (query) => {
    ...
    if (data === "verify") {
        ...
        // YouTube verification - For Railway, you can implement custom logic
    ...
    // Main menu callbacks
        ...
        bot.sendMessage(chatId, getAboutText(year, datetime), { parse_mode: 'Markdown', ...mainMenuKeyboard });
    }
        ...
        const year = config.getCurrentYear();
        const datetime = config.getCurrentDateTime();
        bot.sendMessage(chatId, getStatusText(year, datetime, APIS.length), { parse_mode: 'Markdown', ...mainMenuKeyboard });
    }
...
bot.onText(/\/ping/, async (msg) => {
    ...
    const latency = Date.now() - start;
    bot.editMessageText(`🏓 *PONG!*\n\n📡 Response Time: \`${latency}ms\`\n✅ Status: ONLINE\n🖥️ Platform: Railway`, 
        { chat_id: chatId, message_id: message.message_id, parse_mode: 'Markdown', ...mainMenuKeyboard });
});
